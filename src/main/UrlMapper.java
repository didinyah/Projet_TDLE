...
public class UrlMapper extends Mapper<LongWritable, WARCWritable, Text, LongWritable> {
 	...
 	/**
 	* Count the number of record types found per category (response, request, ...)
	**/
	private void categorizeWARCRecords(WARCWritable value, Context context) throws IOException,InterruptedException {

		String recordType = value.getRecord().getHeader().getRecordType();
        	String targetURL  = value.getRecord().getHeader().getTargetURI();
	      context.write(new Text(recordType), outVal);
 	}
		...
		String recordType = value.getRecord().getHeader().getRecordType();
 		...
 		if (recordType.equals("response")) {
			try {
				...
				String body = content.substring(content.indexOf("<body"));
				...
				while (matcherTag.find()) {
				    ...
				    String url = matcherTag.group();
				    outKey.set(url.toLowerCase());
   ...
   @Override
   /**
    * map - The entrey point of the mapper.
    **/
    public void map(LongWritable key, WARCWritable value, Context context) {

	// -- categorizes WARC Records
	try {
		categorizeWARCRecords(value, context);
	}
	catch (Exception e) {
		System.out.println("* Categorization of WARC Records failed.");
		System.exit(1);
	}
